...
import type { KycVerification, Product, Order } from "@/types";
...
export default function AdminDashboard() {
  ...
  // Fabric upload form
  const [fabricForm, setFabricForm] = useState({ name: "", description: "", price: "", category: "Silk", stock: "", image: "" });
  ...
  const fetchAdminData = useCallback(async () => {
    ...
    try {
      const [kRes, pRes, oRes] = await Promise.all([
        kycService.getQueue(),
  ...
  const handleFabricUpload = async (e: React.FormEvent) => {
    ...
    try {
      await fabricsService.create({
        name: fabricForm.name, description: fabricForm.description,
        price: Number(fabricForm.price), category: fabricForm.category,
        stock: Number(fabricForm.stock),
        image: fabricForm.image || "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?auto=format&fit=crop&w=400&q=80",
      });
      setFabricSuccess(true);
      setFabricForm({ name: "", description: "", price: "", category: "Silk", stock: "", image: "" });
    } catch (err: any) { alert(err.message); }
    ...
    <ProtectedRoute allowedRoles={["ADMIN"]}>
      ...
      <main className="flex-grow w-full max-w-[1280px] mx-auto px-5 md:px-16 pt-32 pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4">
          <div>
            <span className="text-xs font-semibold text-[#5300b7] uppercase tracking-widest block mb-2">System</span>
            <h1 className="font-display text-4xl md:text-5xl font-semibold tracking-tight text-[#1d1a24]">Admin Portal</h1>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {[
            ...
            { label: "KYC Pending", value: kycQueue.length, icon: "pending_actions", color: "text-amber-500" },
        ...
        {tab === "products" && (
          <div className="bg-white rounded-2xl border border-[#ccc3d7]/20 overflow-hidden">
            <div className="px-6 py-4 border-b border-[#ccc3d7]/10">
              <h3 className="font-display text-lg font-semibold text-[#1d1a24]">Product Moderation</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-[#f9f1ff] text-[#4a4455] text-[11px] uppercase font-semibold tracking-wider border-b border-[#ccc3d7]/20">
                ...
                <tbody className="divide-y divide-[#ccc3d7]/10 text-sm">
                  {loading ? [1, 2].map(i => <TableRowSkeleton key={i} />) :
                    prodSlice.length === 0 ? (
                      <tr><td colSpan={5} className="py-12 text-center"><EmptyState icon="inventory_2" title="No pending products" description="All product listings have been reviewed." /></td></tr>
                    ) : prodSlice.map(p => (
                      <tr key={p.id} className="hover:bg-[#f9f1ff]/50 transition-colors">
                        ...
                        <td className="py-4 px-6 text-[#4a4455]">{p.stock} units</td>
                        <td className="py-4 px-6 text-right space-x-2">
            ...
            <Pagination currentPage={productPage} totalPages={Math.ceil(pendingProducts.length / PAGE_SIZE)} onPageChange={setProductPage} />
          </div>
        )}
            ...
            <div className="bg-white rounded-2xl border border-[#ccc3d7]/20 p-6">
              ...
              <form onSubmit={handleFabricUpload} className="flex flex-col gap-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-[10px] uppercase font-semibold text-[#4a4455] block mb-1">Price / yard ($) *</label>
                    <input required type="number" min="0.01" step="0.01" value={fabricForm.price} onChange={e => setFabricForm(f => ({ ...f, price: e.target.value }))} placeholder="45.00" className="w-full text-sm p-3 bg-[#f9f1ff] border border-[#ccc3d7]/40 rounded-xl focus:outline-none focus:border-[#5300b7] focus:bg-white transition-all" />
                  </div>
                  <div>
                    <label className="text-[10px] uppercase font-semibold text-[#4a4455] block mb-1">Stock (yards) *</label>
                  ...
                  <div>
                    <label className="text-[10px] uppercase font-semibold text-[#4a4455] block mb-1">Image URL</label>
                    <input type="url" value={fabricForm.image} onChange={e => setFabricForm(f => ({ ...f, image: e.target.value }))} placeholder="https://…" className="w-full text-sm p-3 bg-[#f9f1ff] border border-[#ccc3d7]/40 rounded-xl focus:outline-none focus:border-[#5300b7] focus:bg-white transition-all" />
                  </div>
                </div>
            ...
            <div className="bg-[#f9f1ff] rounded-2xl border border-[#ccc3d7]/20 p-6 flex flex-col">
              ...
              <div className="space-y-4 flex-grow">
                {[
                  ...
                  { icon: "inventory_2", title: "Stock Accuracy", desc: "Enter the actual yard count. Designers will see this in real-time." },
                  { icon: "payments", title: "Pricing", desc: "Price is per yard USD. Use decimal precision (e.g., 12.50)." },
